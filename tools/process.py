...
# original
parser.add_argument("--gif", type=bool, default=True, help="movie mode")
...
def combine(src, src_path):
    ...
    # remove alpha channel -> change the transparent pixel to white
    if src.shape[2] == 4:
        ...
        for i in range(src.shape[0]):
            for j in range(src.shape[1]):
                if src[i,j,3] == 0.0:
                    ...
                    src[i,j,1] = 1.0
                    src[i,j,2] = 1.0
                    #print("pix:", src[i,j,3], src[i,j,0], src[i,j,1], src[i,j,2])
            #if src[i,j,4]
        src = src[:,:,:3]

    if sibling.shape[2] == 4:
        for i in range(sibling.shape[0]):
            for j in range(sibling.shape[1]):
                if sibling[i,j,3] == 0.0:
                    sibling[i,j,0] = 1.0
                    sibling[i,j,1] = 1.0
                    sibling[i,j,2] = 1.0
        sibling = sibling[:,:,:3]
...
def combine_anime(src, src_path, dst_path):
    if dst_path.path is None:
...
def extract_png_from_apng(file_name, out_dir, image_num):
    im = APNG.open(file_name)
    for i, (png, control) in enumerate(im.frames):
        png.save(out_dir + "/{image_num}-{i}.png".format(image_num=image_num, i=i))
...
def main():
    ...
    #
    #         extract_png_from_apng(src_path, "anime/20", image_num)
    #         image_num += 1
    #
